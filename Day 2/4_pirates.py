...
def naive_scavenging(field):
    ...
    for perm in list_of_perms:
        ...
        for direction in perm:
            if direction == "se":
                # break if it is impossible to move
                if row >= len(field) - 1:
                    break
                else:
                    row +=1
            ...
            col += 1
            total_gold += field[row][col] # 2d notation work backward
...
def dp_scavenging(field): # can solve as a graph problem
    ...
    # because we are required to start in the northwest corner,
    # the max gold collected will be the val at [0][0] and the path start at [0][0]
